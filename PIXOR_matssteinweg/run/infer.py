...
def show_legend(image, ground_truth_color, prediction_color, idx):
    ...
    cv2.putText(
        rectangle, text3, (25, 25 + size3[0][1]), font, font_scale, (100, 100, 100), 1
    )
    cv2.putText(
        rectangle,
        text1,
        (25, 30 + size1[0][1] + size3[0][1]),
        font,
        font_scale,
        ground_truth_color,
        1,
    ...
    cv2.putText(
        rectangle,
        text2,
        (25, 35 + size1[0][1] + size2[0][1] + size3[0][1]),
        font,
        font_scale,
        prediction_color,
        1,
...
def main():
    ...
    for id in ids:
        ...
        final_box_predictions = process_predictions(
            predictions, confidence_threshold=0.5
        )
        ...
        for label in labels:
            ...
            if label.type == "Car":
                ...
                bev_img = kitti_utils.draw_projected_box_bev(
                    bev_image, bbox_corners_camera_coord, color=ground_truth_color
                )
                # display bounding box in camera image
                if bbox_corners_image_coord is not None:
                    camera_image = kitti_utils.draw_projected_box_3d(
                        camera_image, bbox_corners_image_coord, color=ground_truth_color
                    )
        ...
        if final_box_predictions is not None:
            for prediction in final_box_predictions:
                ...
                bbox_y_camera_coord = np.array(
                    [[0.0, 0.0, 0.0, 0.0, 1.65, 1.65, 1.65, 1.65]]
                ).T
                bbox_corners_camera_coord = np.hstack(
                    (bbox_corners_camera_coord, bbox_y_camera_coord)
                )
                switch_indices = np.argsort([0, 2, 1])
                bbox_corners_camera_coord = bbox_corners_camera_coord[:, switch_indices]
                bbox_corners_image_coord = kitti_utils.project_to_image(
                    bbox_corners_camera_coord, calib.P
                )
                ...
                if bbox_corners_image_coord is not None:
                    camera_image = kitti_utils.draw_projected_box_3d(
                        camera_image, bbox_corners_image_coord, color=prediction_color
                    )
        ...
        cv2.imwrite(
            os.path.join(
                config.RESULTS_DIR, f"detections/bev/detection_id_{id}_bev.png"
            ),
            bev_image,
        )
        cv2.imwrite(
            os.path.join(
                config.RESULTS_DIR, f"detections/camera/detection_id_{id}_camera.png"
            ),
            camera_image,
        )
