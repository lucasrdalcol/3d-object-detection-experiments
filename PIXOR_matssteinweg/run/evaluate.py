...
def process_regression_target(valid_reg_predictions, validity_mask):
    ...
    center_x = np.broadcast_to(
        (px_x - valid_reg_predictions[:, 2]).reshape(-1, 1),
        (valid_reg_predictions.shape[0], 4),
    )
    center_y = np.broadcast_to(
        (px_y - valid_reg_predictions[:, 3]).reshape(-1, 1),
        (valid_reg_predictions.shape[0], 4),
    )
    centers = np.hstack((center_x, center_y))

    # get the predicted angles for all bounding boxes
    prediction_angles = np.arctan2(
        valid_reg_predictions[:, 1], valid_reg_predictions[:, 0]
    )
    ...
    rot_matrices = np.stack(
        (prediction_cos, prediction_sin, -prediction_sin, prediction_cos), axis=1
    ).reshape((-1, 2))
    rot_matrices = [
        rot_matrices[i : i + 2, :] for i in range(0, rot_matrices.shape[0], 2)
    ]
...
def perform_nms(valid_class_predictions, valid_box_predictions, nms_threshold):
    ...
    for i in range(sorted_box_predictions.shape[0]):
        ...
        try:
            ...
            ious[i + 1 :] = bbox_iou(
                sorted_box_predictions[i, :], sorted_box_predictions[i + 1 :, :]
            )
        except ValueError:
            break
        except IndexError:
...
def process_predictions(
    ...
    batch_predictions[:, :, :, :-1] = (
        batch_predictions[:, :, :, :-1] * config.REG_STD
    ) + config.REG_MEAN
    ...
    for point_cloud_id in range(batch_predictions.shape[0]):
        ...
        point_cloud_predictions = point_cloud_predictions.reshape(
            (
                config.OUTPUT_DIM_0 * config.OUTPUT_DIM_1,
                config.OUTPUT_DIM_CLA + config.OUTPUT_DIM_REG,
            )
        )
        ...
        if valid_reg_predictions.shape[0]:
            valid_box_predictions = process_regression_target(
                valid_reg_predictions, validity_mask
            )
        else:
            continue
        ...
        if final_batch_predictions is None:
            ...
            final_batch_predictions = np.vstack(
                (final_batch_predictions, final_point_cloud_predictions)
            )
...
def evaluate_model(model, data_loader, distance_ranges, iou_thresholds):
    ...
    eval_dict = {
        distance_range: {
            "targets": {threshold: [] for threshold in iou_thresholds},
            "scores": [],
            "n_labels": 0,
        }
    ...
    for batch_id, (batch_data, batch_labels, batch_calib) in enumerate(data_loader):
        ...
        with torch.set_grad_enabled(False):
            ...
            batch_predictions = np.transpose(
                batch_predictions.cpu().detach().numpy(), (0, 2, 3, 1)
            )
            ...
            for point_cloud_id in range(batch_data.size(0)):
                ...
                if final_box_predictions is not None:
                    point_cloud_predictions = np.vstack(
                        [
                            predictions
                            for predictions in final_box_predictions
                            if predictions[0] == point_cloud_id
                        ]
                    )
                else:
                ...
                for label in batch_labels[point_cloud_id]:
                    ...
                    if label.type == "Car":
                        ...
                        bbox_corners_camera_coord = np.hstack(
                            (
                                bbox_corners_camera_coord[:4, 0],
                                bbox_corners_camera_coord[:4, 2],
                            )
                        )
                        if ground_truth_box_corners is None:
                            ...
                            ground_truth_box_corners = np.vstack(
                                (ground_truth_box_corners, bbox_corners_camera_coord)
                            )

                assert np.all(
                    np.diff(distance_ranges) <= 0
                )  # check that distance ranges monotonically decrease
                # iterate over all distance ranges
                for distance_range in distance_ranges:

                    # valid predictions and labels exist for the currently inspected point cloud
                    if (
                        ground_truth_box_corners is not None
                        and point_cloud_predictions is not None
                    ):

                        # remove all predictions and labels outside of the specified range
                        max_distance_predictions = np.max(
                            point_cloud_predictions[:, 6:], axis=1
                        )
                        max_distance_ground_truth = np.max(
                            ground_truth_box_corners[:, 4:], axis=1
                        )
                        distance_mask_predictions = np.where(
                            max_distance_predictions <= distance_range, True, False
                        )
                        distance_mask_ground_truth = np.where(
                            max_distance_ground_truth <= distance_range, True, False
                        )
                        point_cloud_predictions = point_cloud_predictions[
                            distance_mask_predictions
                        ]
                        ground_truth_box_corners = ground_truth_box_corners[
                            distance_mask_ground_truth
                        ]

                        # valid predictions and labels exist inside the specified range
                        if (
                            point_cloud_predictions.shape[0]
                            and ground_truth_box_corners.shape[0]
                        ):
                            ...
                            ious = np.zeros(
                                (
                                    point_cloud_predictions.shape[0],
                                    ground_truth_box_corners.shape[0],
                                )
                            )
                            confidence_scores = np.zeros(
                                (
                                    point_cloud_predictions.shape[0],
                                    ground_truth_box_corners.shape[0],
                                )
                            ...
                            for pid, prediction in enumerate(point_cloud_predictions):
                                ious[pid, :] = bbox_iou(
                                    prediction[2:], ground_truth_box_corners
                                )
                                confidence_scores[pid, np.argmax(ious[pid, :])] = (
                                    prediction[1]
                                )
                            ...
                            for threshold in iou_thresholds:
                                eval_dict[distance_range]["targets"][threshold].extend(
                                    np.where(np.max(ious, axis=1) > threshold, 1, 0)
                                )

                            # store the predictions' confidence scores and the number of labels
                            eval_dict[distance_range]["scores"].extend(
                                np.max(confidence_scores, axis=1)
                            )
                            eval_dict[distance_range][
                                "n_labels"
                            ] += ground_truth_box_corners.shape[0]
                        ...
                        # valid predictions exist but no labels
                            ...
                            point_cloud_predictions.shape[0]
                            and not ground_truth_box_corners.shape[0]
                        ):

                            # store all predictions as negative detections
                            for threshold in iou_thresholds:
                                eval_dict[distance_range]["targets"][threshold].extend(
                                    np.zeros((point_cloud_predictions.shape[0],))
                                )

                            # store the predictions' confidence scores
                            eval_dict[distance_range]["scores"].extend(
                                point_cloud_predictions[:, 1]
                            )

                        # valid labels exist but no predictions
                        elif (
                            not point_cloud_predictions.shape[0]
                            and ground_truth_box_corners.shape[0]
                        ):
                    ...
                    # valid predictions exist for the currently inspected point cloud but no labels
                        ...
                        ground_truth_box_corners is None
                        and point_cloud_predictions is not None
                    ):
                    ...
                    # valid labels exist for the currently inspected point cloud but no predictions
                        ...
                        point_cloud_predictions is None
                        and ground_truth_box_corners is not None
                    ):

                        # remove all labels outside of the specified range
                        max_distance_ground_truth = np.max(
                            ground_truth_box_corners[:, 6:], axis=1
                        )
                        distance_mask_ground_truth = np.where(
                            max_distance_ground_truth <= distance_range, True, False
                        )
                ...
                print("++++++++++++++++++++++++++++++")
                print(
                    "Analyze Point Cloud {:d}/{:d}".format(
                        batch_id * config.BATCH_SIZE + point_cloud_id + 1,
    ...
    for distance_range in distance_ranges:
        ...
        eval_dict[distance_range]["scores"] = np.array(
            eval_dict[distance_range]["scores"]
        )[sorted_indices]
        ...
        for threshold in iou_thresholds:
            ...
            eval_dict[distance_range]["targets"][threshold] = np.array(
                eval_dict[distance_range]["targets"][threshold]
            )[sorted_indices]
            ...
            # compute recall
            recall = list(
                np.cumsum(eval_dict[distance_range]["targets"][threshold])
                / eval_dict[distance_range]["n_labels"]
            )
            recall.insert(0, 0.0)  # start with 0
            recall = np.array(recall)  # convert to numpy array for further processing
            performance_dict["recall"] = recall

            # compute precision
            precision = [
                np.sum(eval_dict[distance_range]["targets"][threshold][: i + 1])
                / (i + 1)
                for i in range(len(eval_dict[distance_range]["targets"][threshold]))
            ]
            precision.insert(0, 0.0)  # start with 0
        ...
        average_precisions = [
            eval_dict[distance_range][key]["AP"]
            for key in eval_dict[distance_range]
            if not isinstance(key, str)
        ]
        eval_dict[distance_range]["mAP"] = sum(average_precisions) / len(
            average_precisions
        )
